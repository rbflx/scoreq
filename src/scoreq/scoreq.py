...
class Scoreq:
    ...
    def _init_onnx(self):
        ...
        if not model_url:
            raise ValueError(
                f"Invalid model combination: domain='{self.data_domain}', mode='{self.mode}'"
            )

        model_path = self._download_model(
            onnx_filename, model_url, cache_dir_name="onnx-models"
        )
    ...
    def _init_pytorch(self):
        ...
        PT_URLS = {
            ...
            (
                "natural",
                "ref",
            ): "https://zenodo.org/records/13860326/files/fixed_nmr_telephone.pt",
            (
                "synthetic",
                "nr",
            ): "https://zenodo.org/records/13860326/files/adapt_nr_synthetic.pt",
            (
                "synthetic",
                "ref",
            ): "https://zenodo.org/records/13860326/files/fixed_nmr_synthetic.pt",
        }
        ...
        MODEL_PATH = self._download_model(
            os.path.basename(model_url), model_url, "pt-models"
        )
    ...
    def _download_model(self, filename, url, cache_dir_name):
        ...
        cache_dir = os.path.join(
            os.path.expanduser("~"), ".cache", "scoreq", cache_dir_name
        )
    ...
    def predict(self, test_path, ref_path=None):
        ...
        if self.mode == "ref":
            if ref_path is None:
                raise ValueError("ref_path must be provided for reference mode.")
            ref_wave_raw = self.load_processing(ref_path)
            ref_wave_padded = dynamic_pad(ref_wave_raw).numpy()
        else:
            ref_wave_padded = None

        if self.use_onnx:
            return self._predict_onnx(test_wave_padded, ref_wave_padded)
        else:
            return self._predict_pytorch(test_wave_padded, ref_wave_padded)

    def predict_audio(
        self,
        test_wave_raw: torch.Tensor,
        test_sr: int,
        ref_wave_raw: torch.Tensor | None = None,
        ref_sr: int | None = None,
    ):
        # both waves: (c, t)
        test_wave_processed = self.process_audio(test_wave_raw, test_sr)
        test_wave_padded = dynamic_pad(test_wave_processed).numpy()

        if self.mode == "ref":
            if ref_wave_raw is None or ref_sr is None:
                raise ValueError(
                    "ref_wave_raw and ref_sr must be provided for reference mode."
                )
            ref_wave_processed = self.process_audio(ref_wave_raw, ref_sr)
            ref_wave_padded = dynamic_pad(ref_wave_processed).numpy()
        else:
            ref_wave_padded = None
    ...
    def _predict_onnx(self, test_wave_padded, ref_wave_padded=None):
        ...
        if self.mode == "nr":
            ...
            assert (
                ref_wave_padded is not None
            ), "ref_wave_padded must be provided for reference mode."
    ...
    def _predict_pytorch(self, test_wave_padded, ref_wave_padded=None):
    ...
    def process_audio(self, wave: torch.Tensor, orig_sr: int, target_sr: int = 16_000):
        # wave: (c, t)
        if wave.shape[0] > 1:
    ...
    def load_processing(self, filepath, target_sr=16000):
        ...
        wave = self.process_audio(wave, sr, target_sr)
        return wave
